  ...
  <div class="app-container">
    ...
    <el-row>
      <div class="chart-container">
        <el-table v-loading="loading" ref="ShoppingCartTable" :data="data" border fit highlight-current-row style="width: 100%;" @sort-change="sortSignTime" @row-dblclick="openDetails">
          <!-- 操作 -->
          ...
          <el-table-column :sortable="'custom'" prop="nums" label="数量" align="center">
            <template slot-scope="scope">
              {{ scope.row.nums }}
            </template>
          </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="48%" top="7vh">
      <el-form ref="shoppingCartForm" :model="shoppingCartTemp" :rules="rules" class="form-add" label-width="100px" style="width: 100%; padding-left:10px;padding-right:10px">
        <el-form-item label="数量" prop="nums">
          <el-input-number v-model="shoppingCartTemp.nums" style="width:100%"/>
        </el-form-item>
      </el-form>
...
<script>
import { listShoppingCart, createShoppingCart, updateShoppingCart, deleteShoppingCart } from '@/api/ShoppingCartApi'
...
export default {
  ...
  methods: {
    list() {
      ...
      this.loading = true
      listShoppingCart(params).then(response => {
        this.loading = false
    ...
    resetshoppingCartTemp() {
      this.shoppingCartTemp = {
        nums: undefined,
        id: undefined
      }
    },
    handleCreate() {
      this.resetshoppingCartTemp()
      this.dialogStatus = 'create'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['shoppingCartForm'].clearValidate()
      })
    },
    createData() {
      this.$refs['shoppingCartForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          createShoppingCart(this.shoppingCartTemp).then(response => {
    ...
    handleUpdate(row) {
      Object.assign(this.shoppingCartTemp, row)
    ...
    updateData() {
      this.$refs['shoppingCartForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          updateShoppingCart(this.shoppingCartTemp.id, this.shoppingCartTemp).then(response => {
            this.buttonLoading = false
            if (response.data.code === -1) {
              this.$message({
                title: '失败',
    ...
    deleteData(id) {
      this.$confirm('确认删除该购物车商品信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteShoppingCart(id).then(response => {
          if (response.data.code === -1) {
            this.$message({
              title: '失败',
              message: response.data.message,
