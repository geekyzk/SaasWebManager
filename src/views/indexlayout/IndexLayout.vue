  ...
  <div class="app-container">
    ...
    <el-row>
      <div class="chart-container">
        <el-table v-loading="loading" ref="IndexLayoutTable" :data="data" border fit highlight-current-row style="width: 100%;" @sort-change="sortSignTime" @row-dblclick="openDetails">
          <!-- 操作 -->
          ...
          <el-table-column :sortable="'custom'" prop="name" label="布局值" align="center">
            <template slot-scope="scope">
              {{ scope.row.name }}
            </template>
          </el-table-column>
          <el-table-column :sortable="'custom'" prop="label" label="布局名" align="center">
            <template slot-scope="scope">
              {{ scope.row.label }}
            </template>
          </el-table-column>
          <el-table-column :sortable="'custom'" prop="remark" label="备注" align="center">
            <template slot-scope="scope">
              {{ scope.row.remark }}
            </template>
          </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="48%" top="7vh">
      <el-form ref="indexLayoutForm" :model="indexLayoutTemp" :rules="rules" class="form-add" label-width="100px" style="width: 100%; padding-left:10px;padding-right:10px">
        <el-form-item label="布局值" prop="name">
          <el-input v-model="indexLayoutTemp.name" style="width:100%"/>
        </el-form-item>
        <el-form-item label="布局名" prop="label">
          <el-input v-model="indexLayoutTemp.label" style="width:100%"/>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input v-model="indexLayoutTemp.remark" style="width:100%"/>
        </el-form-item>
      </el-form>
    ...
    <el-dialog :visible.sync="dialogDetailVisible" title="详情" width="40%" top="7vh" style="width: 100%; padding-left:10px;padding-right:10px">
      <el-form ref="form" :model="showData" label-width="90px" label-position="left" style="padding-left: 20px">
        <el-form-item label="布局值">
          <span> {{ showData.name }}</span>
        </el-form-item >
        <el-form-item label="布局名">
...
<script>
import { listIndexLayout, createIndexLayout, updateIndexLayout, deleteIndexLayout } from '@/api/IndexLayoutApi'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: '布局值为必须参数', trigger: 'change' }],
        label: [{ required: true, message: '布局名为必须参数', trigger: 'change' }],
        remark: [{ required: true, message: '备注为必须参数', trigger: 'change' }]
      }
    }
  },
  created() {
  ...
  methods: {
    list() {
      ...
      this.loading = true
      listIndexLayout(params).then(response => {
        this.loading = false
    ...
    resetindexLayoutTemp() {
      this.indexLayoutTemp = {
        name: undefined,
        label: undefined,
        remark: undefined,
        id: undefined
    ...
    handleCreate() {
      this.resetindexLayoutTemp()
      this.dialogStatus = 'create'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['indexLayoutForm'].clearValidate()
      })
    },
    createData() {
      this.$refs['indexLayoutForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          createIndexLayout(this.indexLayoutTemp).then(response => {
    ...
    handleUpdate(row) {
      Object.assign(this.indexLayoutTemp, row)
    ...
    updateData() {
      this.$refs['indexLayoutForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          updateIndexLayout(this.indexLayoutTemp.id, this.indexLayoutTemp).then(response => {
            this.buttonLoading = false
            if (response.data.code === -1) {
              this.$message({
                title: '失败',
    ...
    deleteData(id) {
      this.$confirm('确认删除该首页布局, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteIndexLayout(id).then(response => {
          if (response.data.code === -1) {
            this.$message({
              title: '失败',
              message: response.data.message,
