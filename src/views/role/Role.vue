  ...
  <div class="app-container">
        ...
        <el-form>
          ...
          <el-col :span="4">
            <el-form-item label="名称" label-width="50px">
              <el-input v-model="listParams['name.contains']" style="width: 100%;" class="filter-item" placeholder="名称"/>
            </el-form-item>
          </el-col>
          <el-col :span="2">
    ...
    <el-row>
      <div class="chart-container">
        <el-table v-loading="loading" :data="data" border fit highlight-current-row style="width: 100%;" @sort-change="sortSignTime" @row-dblclick="openDetails">
          <!-- 操作 -->
          ...
          <el-table-column :sortable="'custom'" prop="name" label="角色名称" align="center">
            <template slot-scope="scope">
              {{ scope.row.name }}
            </template>
          </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="38%" top="7vh">
      ...
      <el-tree
        ...
        node-key="id"
      />
      <button @click="getCheckedKeys">测试</button>
...
<script>
import { listRole, createRole, updateRole, deleteRole } from '@/api/RoleApi'
import { listMenuPermission } from '@/api/PermissionApi'
...
export default {
  ...
  data() {
    return {
      ...
      allPermissions: [],
      rules: {
        name: [{ required: true, message: '角色名称为必须参数', trigger: 'change' }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    createData() {
      this.$refs['roleForm'].validate((valid) => {
        if (valid) {
          ...
          const reqBody = Object.assign({}, this.roleTemp)
          reqBody['permissions'] = this.$refs.tree.getCheckedKeys()
          createRole(reqBody).then(response => {
    ...
    handleUpdate(row) {
      Object.assign(this.roleTemp, row)
    ...
    deleteData(id) {
      this.$confirm('确认删除该角色信息, 是否继续?', '提示', {
