  ...
  <div class="app-container">
        ...
        <el-form>
          ...
          <el-col :span="4">
            <el-form-item label="图片" label-width="80px">
              <el-input
                v-model="listParams['image.equals']"
                style="width: 100%;"
                class="filter-item"
                placeholder="图片"
    ...
    <el-row>
      <div class="chart-container">
        <el-table v-loading="loading" ref="GoodsImageTable" :data="data" border fit highlight-current-row style="width: 100%;" @sort-change="sortSignTime" @row-dblclick="openDetails">
          <!-- 操作 -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="48%" top="7vh">
      <el-form ref="goodsImageForm" :model="goodsImageTemp" :rules="rules" class="form-add" label-width="100px" style="width: 100%; padding-left:10px;padding-right:10px">
        <el-form-item label="图片" prop="image">
          <el-input v-model="goodsImageTemp.image" style="width:100%"/>
        </el-form-item>
      </el-form>
...
<script>
import { listGoodsImage, createGoodsImage, updateGoodsImage, deleteGoodsImage } from '@/api/GoodsImageApi'
  ...
  methods: {
    list() {
      const params = {
        ...
        'image.equals': this.listParams['image.equals'] === '' ? undefined : this.listParams['image.equals'],
        'sort': this.listParams['sort'] === '' ? null : this.listParams['sort']
      }
      this.loading = true
      listGoodsImage(params).then(response => {
        this.loading = false
    ...
    handleCreate() {
      this.resetgoodsImageTemp()
      this.dialogStatus = 'create'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['goodsImageForm'].clearValidate()
      })
    },
    createData() {
      this.$refs['goodsImageForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          createGoodsImage(this.goodsImageTemp).then(response => {
    ...
    handleUpdate(row) {
      Object.assign(this.goodsImageTemp, row)
    ...
    updateData() {
      this.$refs['goodsImageForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          updateGoodsImage(this.goodsImageTemp.id, this.goodsImageTemp).then(response => {
            this.buttonLoading = false
            if (response.data.code === -1) {
              this.$message({
                title: '失败',
    ...
    deleteData(id) {
      this.$confirm('确认删除该商品图片, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteGoodsImage(id).then(response => {
          if (response.data.code === -1) {
            this.$message({
              title: '失败',
              message: response.data.message,
