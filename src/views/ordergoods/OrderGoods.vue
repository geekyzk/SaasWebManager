  ...
  <div class="app-container">
    ...
    <el-row>
      <div class="chart-container">
        <el-table v-loading="loading" ref="OrderGoodsTable" :data="data" border fit highlight-current-row style="width: 100%;" @sort-change="sortSignTime" @row-dblclick="openDetails">
          <!-- 操作 -->
          ...
          <el-table-column :sortable="'custom'" prop="goodsNum" label="商品数量" align="center">
            <template slot-scope="scope">
              {{ scope.row.goodsNum }}
            </template>
          </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="48%" top="7vh">
      <el-form ref="orderGoodsForm" :model="orderGoodsTemp" :rules="rules" class="form-add" label-width="100px" style="width: 100%; padding-left:10px;padding-right:10px">
        <el-form-item label="商品数量" prop="goodsNum">
          <el-input-number v-model="orderGoodsTemp.goodsNum" style="width:100%"/>
        </el-form-item>
      </el-form>
...
<script>
import { listOrderGoods, createOrderGoods, updateOrderGoods, deleteOrderGoods } from '@/api/OrderGoodsApi'
...
export default {
  ...
  methods: {
    list() {
      ...
      this.loading = true
      listOrderGoods(params).then(response => {
        this.loading = false
    ...
    handleCreate() {
      this.resetorderGoodsTemp()
      this.dialogStatus = 'create'
      this.dialogFormVisible = true
      this.$nextTick(() => {
        this.$refs['orderGoodsForm'].clearValidate()
      })
    },
    createData() {
      this.$refs['orderGoodsForm'].validate((valid) => {
        if (valid) {
          ...
          createOrderGoods(this.orderGoodsTemp).then(response => {
    ...
    handleUpdate(row) {
      Object.assign(this.orderGoodsTemp, row)
    ...
    updateData() {
      this.$refs['orderGoodsForm'].validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          updateOrderGoods(this.orderGoodsTemp.id, this.orderGoodsTemp).then(response => {
            this.buttonLoading = false
            if (response.data.code === -1) {
              this.$message({
                title: '失败',
    ...
    deleteData(id) {
      this.$confirm('确认删除该订单商品信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteOrderGoods(id).then(response => {
          if (response.data.code === -1) {
            this.$message({
              title: '失败',
              message: response.data.message,
